...
st.title('Analyze and compare continuous Profile')
...
if file_bp:
    ...
    if show_rawdata:
        with expander_options:
            number_pulses_maximum = st.slider("Number of measurements used for maximum of rawdata:", min_value=1, max_value=100, value=50)
    ...
    ext_file = st.file_uploader("Choose another Profile to compare", key='ext_bp')
    ...
    if ext_file and smoothfactor != 0:
        ...
        # create the data frame
        # df_ext = pd.read_csv(ext_file, skiprows=0, sep='\t\t', engine='python')
        # df_ext['PDD'] = df_ext.Dose / df_ext.Dose.max() * 100
        #
        # fig1.add_scatter(
        #     x=df_ext['Z'],
        #     y=df_ext['PDD'],
        ...
        # Find rows to skip
        for number, line in enumerate(ext_file):
            if line.startswith(b'Inline;'):
                break

        for number2, line in enumerate(ext_file):
            if line.startswith(b'Inline;'):
                break
        ext_file.seek(0)

        # Find axis of Profile
        for line_axis in ext_file:
            if line_axis.startswith(b'Scan type:'):
                if line_axis.startswith(b'Scan type:; Crossline'):
                    axis_used_mayo = ' Crossline'
                else:
                    axis_used_mayo = 'Inline'
                break
        ext_file.seek(0)

        df_ext_1 = pd.read_csv(ext_file, skiprows=number, nrows=number2-number, sep=';', engine='python')
        ext_file.seek(0)

        df_ext_2 = pd.read_csv(ext_file, skiprows=number+number2+1, sep=';', engine='python')

        df_ext_1['pdd'] = df_ext_1[' Ratio'] / df_ext_1[' Ratio'].max() * 100
        df_ext_2['pdd'] = df_ext_2[' Ratio'] / df_ext_2[' Ratio'].max() * 100


        fig1.add_scatter(
            x=df_ext_1[axis_used_mayo],
            y=df_ext_1['pdd'],
            mode='markers',
            name=f'First {axis_used_mayo} Profile',
            marker=dict(color='yellow')
        )

        fig1.add_scatter(
            x=df_ext_2[axis_used_mayo],
            y=df_ext_2['pdd'],
            mode='markers',
            name=f'Second {axis_used_mayo} Profile',
        ...
        if show_gamma:
            ...
            positions_ref = (np.array(df_ext_2[axis_used_mayo]),)
            doses_ref = np.array(df_ext_2['pdd'])
